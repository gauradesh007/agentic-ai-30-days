import json
import requests
from datetime import datetime

# Local Ollama API endpoint and model.
OLLAMA_URL = "http://localhost:11434/api/chat"
MODEL = "llama3.2:1b"


# -------------------
# TOOL FUNCTIONS
# -------------------


def calculator(expression: str) -> str:
    """
    Executes a simple math expression.

    Example:
    "0.12 * 7500" -> "900.0"
    """

    try:
        expression = expression.strip()
        expression = expression.replace("%", "/100")

        allowed = "0123456789+-*/(). "

        if not all(char in allowed for char in expression):
            return "ERROR: Invalid expression"

        return str(eval(expression))

    except Exception as e:
        return f"ERROR: {e}"


def current_date(_: str = "") -> str:
    """
    Returns the actual current system date.

    The unused argument keeps the tool interface consistent:
    every tool accepts one input string.
    """

    return datetime.now().strftime("%A, %d %B %Y")


def text_length(text: str) -> str:
    """
    Returns the character count of the input text.

    Example:
    "Agentic" -> "7"
    """

    return str(len(text))


# -------------------
# TOOL REGISTRY
# -------------------

TOOL_REGISTRY = {
    "calculator": {
        "function": calculator,
        "description": "Performs simple math calculations.",
        "input_type": "plain math expression string",
        "example": "0.12 * 7500",
    },
    "current_date": {
        "function": current_date,
        "description": "Returns today's current system date.",
        "input_type": "empty string",
        "example": "",
    },
    "text_length": {
        "function": text_length,
        "description": "Counts characters in a text string.",
        "input_type": "text string",
        "example": "Agentic",
    },
}


def describe_tools() -> str:
    """
    Converts the tool registry into readable tool descriptions
    for the system prompt.
    """

    lines = []

    for name, spec in TOOL_REGISTRY.items():
        lines.append(
            f"- {name}: {spec['description']} "
            f"Input type: {spec['input_type']}. "
            f"Example input: {spec['example']}"
        )

    return "\n".join(lines)


# -------------------
# OLLAMA CALL
# -------------------


def call_ollama(messages):
    """
    Sends conversation history to Ollama and returns model output text.
    """

    response = requests.post(
        OLLAMA_URL,
        json={
            "model": MODEL,
            "messages": messages,
            "stream": False,
            "options": {"temperature": 0},
        },
        timeout=600,
    )

    response.raise_for_status()
    return response.json()["message"]["content"]


# -------------------
# JSON EXTRACTION
# -------------------


def extract_json_objects(text: str):
    """
    Extracts valid JSON objects from messy local model output.

    The model may return plain text, multiple JSON objects,
    or malformed explanations. This function extracts usable JSON only.
    """

    text = text.strip()
    objects = []

    stack = 0
    start = None

    for i, char in enumerate(text):
        if char == "{":
            if stack == 0:
                start = i
            stack += 1

        elif char == "}":
            stack -= 1

            if stack == 0 and start is not None:
                candidate = text[start : i + 1]

                try:
                    objects.append(json.loads(candidate))
                except:
                    pass

    return objects


# -------------------
# AGENT STATE
# -------------------

user_question = (
    "What day is it today, what is 12% of 7500, "
    "and how many characters are in Agentic?"
)

# Tools required to fully answer the question.
required_tools = ["current_date", "calculator", "text_length"]

# Stores successful tool outputs.
tool_results = {}

# Stores model/controller reasoning traces.
thought_history = []

MAX_STEPS = 8


# -------------------
# SYSTEM PROMPT
# -------------------

tool_descriptions = describe_tools()

messages = [
    {
        "role": "system",
        "content": f"""
You are a tool-aware AI workflow agent.

You must select the best available tool for the task.

Available tools:
{tool_descriptions}

Always return EXACTLY ONE JSON object.

Format:

{{
  "thought": "reasoning",
  "action": "tool_name",
  "input": "tool input"
}}

OR

{{
  "thought": "reasoning",
  "action": "final",
  "answer": "final response"
}}

Rules:
- Never explain outside JSON
- Use only tools listed above
- Use one tool at a time
- Use calculator for math
- Use current_date for today's date
- Use text_length for counting characters
""",
    },
    {
        "role": "user",
        "content": user_question,
    },
]


# -------------------
# CONTROLLER FUNCTIONS
# -------------------


def choose_action(json_objects):
    """
    Selects the first valid model-suggested action.

    The controller only accepts:
    - tools listed in TOOL_REGISTRY
    - final response
    """

    for obj in json_objects:
        action = obj.get("action")

        if action in TOOL_REGISTRY:
            return obj

        if action == "final":
            return obj

    return None


def get_next_missing_tool(required_tools, tool_results):
    """
    Returns the next required tool that has not completed yet.
    """

    for tool in required_tools:
        if tool not in tool_results:
            return tool

    return None


def fallback_action_for_tool(tool_name):
    """
    Controller fallback when the model:
    - refuses incorrectly
    - chooses the wrong tool
    - repeats a completed tool
    - returns invalid JSON
    """

    if tool_name == "current_date":
        return {
            "thought": "Current date is still missing, so the controller is selecting current_date.",
            "action": "current_date",
            "input": "",
        }

    if tool_name == "calculator":
        return {
            "thought": "Calculator result is still missing, so the controller is selecting calculator.",
            "action": "calculator",
            "input": "0.12 * 7500",
        }

    if tool_name == "text_length":
        return {
            "thought": "Text length result is still missing, so the controller is selecting text_length.",
            "action": "text_length",
            "input": "Agentic",
        }

    return None


# -------------------
# DYNAMIC TOOL LOOP
# -------------------

for step in range(MAX_STEPS):

    print(f"\n--- STEP {step + 1} ---")

    # Give the model current workflow state.
    messages.append(
        {
            "role": "user",
            "content": f"""
Original question:
{user_question}

Required tools:
{required_tools}

Completed tool results:
{tool_results}

Thought history:
{thought_history}

Choose the next missing tool.
Return exactly one JSON object.
""",
        }
    )

    # Ask the model for the next action.
    decision_text = call_ollama(messages)

    print("\nMODEL OUTPUT:")
    print(decision_text)

    # Extract structured JSON objects.
    json_objects = extract_json_objects(decision_text)

    # Controller selects the first valid action.
    decision = choose_action(json_objects)

    # Determine which required tool is still missing.
    next_missing_tool = get_next_missing_tool(
        required_tools,
        tool_results,
    )

    if not next_missing_tool:
        print("All required tools completed.")
        break

    # If model output is unusable,
    # the controller selects a fallback action.
    if not decision:

        print("\nNo valid model action found. " "Controller will choose fallback.")

        decision = fallback_action_for_tool(next_missing_tool)

    # Defensive validation for Pylance/runtime safety.
    if decision is None:
        break

    action = decision.get("action")
    thought = decision.get("thought", "")
    tool_input = decision.get("input", "")

    # Prevent repeated execution of completed tools.
    if action in tool_results:

        print(
            f"\nTool {action} already completed. "
            "Controller will choose next missing tool."
        )

        decision = fallback_action_for_tool(next_missing_tool)

        if decision is None:
            break

        action = decision.get("action")
        thought = decision.get("thought", "")
        tool_input = decision.get("input", "")

    # Validate that the selected action matches
    # the expected next workflow tool.
    if action != next_missing_tool:

        print("\nACTION MISMATCH:")
        print(f"Expected next missing tool: {next_missing_tool}")
        print(f"Model selected: {action}")

        decision = fallback_action_for_tool(next_missing_tool)

        if decision is None:
            break

        action = decision.get("action")
        thought = decision.get("thought", "")
        tool_input = decision.get("input", "")

    print("\nTHOUGHT:")
    print(thought)

    thought_history.append(thought)

    # Stop if workflow is complete.
    if action == "final":
        break

    # Execute valid tool.
    if action in TOOL_REGISTRY:

        print(f"\nACTION: {action}")
        print("INPUT:", tool_input)

        tool_function = TOOL_REGISTRY[action]["function"]

        try:
            result = tool_function(tool_input)

        except Exception as e:
            result = f"ERROR: {e}"

        print("\nOBSERVATION:")
        print(result)

        # Store successful results only.
        if not result.startswith("ERROR"):
            tool_results[action] = result

        else:
            print("Tool failed. Result not stored.")

    else:
        print("Invalid tool selected:", action)
        break
# -------------------
# FINAL SUMMARY
# -------------------

print("\n--- FINAL STATE ---")

print("\nTOOL RESULTS:")
print(tool_results)

print("\nTHOUGHT HISTORY:")
for i, thought in enumerate(thought_history, start=1):
    print(f"{i}. {thought}")

if all(tool in tool_results for tool in required_tools):
    print(
        f"\nFINAL CONTROLLER ANSWER:\n"
        f"Today is {tool_results['current_date']}.\n"
        f"12% of 7500 is {tool_results['calculator']}.\n"
        f"The word Agentic has {tool_results['text_length']} characters."
    )
else:
    print("\nAgent did not complete all required tools.")
